...
class SearchViewModelTest {
    ...
    fun searchNotEmptyList() {
        Mockito.`when`(searchRepository.search(ArgumentMatchers.anyString())).thenReturn(Observable.just(mockList()))
    ...
    @Test
    fun searchEmptyList() {
        Mockito.`when`(searchRepository.search(ArgumentMatchers.anyString())).thenReturn(Observable.just(emptyList()))
        viewModel.search("query")
